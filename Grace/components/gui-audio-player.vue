	...
	<view class="gui-player">
		...
		@tap.stop.parent="hideList" @touchmove.stop.parent="">
			...
			:style="{background:listBg, height:listHeight}">
				<view class="graceplayer-list-item">{{$t('gui_audio_player_4')}}</view>
...
export default{
	...
	methods:{
		...
		next : function () {
			if(this.index + 1 >= this.list.length){uni.showToast({title:this.$t('gui_audio_player_5'), icon:"none"}); return ;}
			this.index++;
			this.play();
		},
		prev : function () {
			if(this.index -1 < 0){uni.showToast({title:this.$t('gui_audio_player_6'), icon:"none"}); return ;}
			this.index--;
			this.play();
		},
