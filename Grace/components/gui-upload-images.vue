	...
	<view class="gui-flex gui-rows gui-wrap">
		...
		v-for="(item, index) in imgLists" :key="index">
			...
			@tap.stop="retry" :data-index="index" v-if="item.error">
				...
				<text class="gui-add-list-reup-text gui-color-white">{{$t('gui_upload_images_116')}}</text>
			</view>
			<!-- 删除 -->
...
export default {
	...
	props : {
		...
		btnName         : { type : String, default : this.$t('gui_upload_images_117') },
    ...
    methods : {
		...
		upload       : function(index){
			...
			this.updatting   = true;
			if(!index){index = 0;}
			uni.showLoading({title:this.$t('gui_upload_images_118'), mask:true});
		...
		uploadBase : function (index) {
			...
			if(this.uploadServerUrl == ''){
				uni.showToast({title:this.$t('gui_upload_images_119'), icon:"none"});
				return ;
			}
			// 检查是否是默认值
			...
			const task = uni.uploadFile({
				...
				success  : (uploadRes) => {
					try{
						...
						if(uploadRes.status != 'ok'){
							uni.showToast({title:this.$t('gui_upload_images_120')+uploadRes.data, icon:"none"});
							this.error(index);
						}else{
							//上传图片成功
				...
				fail    : (e) => {
					uni.showToast({title:this.$t('gui_upload_images_121'), icon:"none"});
					this.error(index);
		...
		upload2cloud:function (index) {
			...
			uniCloud.uploadFile({
				...
				fail : ()=>{
					uni.showToast({title:this.$t('gui_upload_images_122'), icon:"none"});
					this.error(index);
				}
			});
		}
