...
<gui-popup 
	...
	@tap.stop.prevent="stopfun">
		...
		<view class="gui-calendar-header gui-flex gui-rows gui-nowrap gui-justify-content-center gui-align-items-center">
			...
			<text class="gui-calendar-header-btn gui-icons" 
			style="margin:20rpx;">{{$t('gui_calendar_7', [cYear,cMonth])}}</text>
			<text class="gui-calendar-header-btn gui-icons" 
		...
		v-if="isTime">
			<picker mode="time" @change="timechange" :value="currentTimeIn">
				...
				style="border-color:#D1D1D1;" >{{$t('gui_calendar_8', [currentTimeIn])}}</text>
			</picker>
		</view>
		<!-- 按钮 -->
		<view class="gui-flex gui-rows gui-space-between gui-align-items-center">
			<view class="gui-date-btns-text" hover-class="gui-tap" @tap="close">
				<text class="gui-date-btns-text gui-block-text gui-color-gray">{{$t('gui_calendar_9')}}</text>
			</view>
			<view class="gui-date-btns-text" hover-class="gui-tap" @tap="submit">
				<text class="gui-date-btns-text gui-block-text gui-primary-color">{{$t('gui_calendar_10')}}</text>
			</view>
		</view>
	</view>
</gui-popup>
...
export default {
	...
	data(){
		return {
			weeks         : [this.$t('gui_calendar_11'), this.$t('gui_calendar_12'), this.$t('gui_calendar_13'), this.$t('gui_calendar_14'), this.$t('gui_calendar_15'), this.$t('gui_calendar_16'), this.$t('gui_calendar_17')],
			cYear         : 2016,
			cMonth        : 6,
			cMonthStr     : "06",
			cDay          : "01",
