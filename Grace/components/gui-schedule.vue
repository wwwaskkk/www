<template>
	<view class="gui-schedule-wrap">
		<view class="gui-flex gui-rows gui-space-between gui-align-items-center">
			...
			@change="selectDate">
				<text 
				class="gui-schedule-header-date gui-icons">{{$t('gui_schedule_98', [cYear,cMonthStr])}}</text>
			</picker>
			<text class="gui-border gui-schedule-today" @tap="gotoToday">{{$t('gui_schedule_99')}}</text>
		</view>
...
export default{
	...
	data() {
		return {
			...
			cDay       : 10,
			cMonthStr  : '01',
			weeks      : [this.$t('gui_schedule_100'), this.$t('gui_schedule_101'), this.$t('gui_schedule_102'), this.$t('gui_schedule_103'), this.$t('gui_schedule_104'), this.$t('gui_schedule_105'), this.$t('gui_schedule_106')],
			days       : [],
			currentDayIn : '',
