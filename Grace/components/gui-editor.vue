...
<view class="gui-editor">
	<view class="gui-border-b">
		...
		v-model="article.title" :placeholder="$t('gui_editor_59')" auto-height />
	</view>
	<!-- 空内容提示 -->
	<view v-if="article.contents.length < 1">
		<text class="gui-color-gray gui-editor-empty">{{$t('gui_editor_60')}}</text>
	</view>
	<!-- 内容区域 -->
	...
	class="gui-editor-items">
		...
		style="background-color:#F8F8F8; padding:20rpx;">
			...
			:value="item.content" :placeholder="$t('gui_editor_63')" />
		</view>
		<!-- 图片 -->
		...
		class="gui-editor-img-wrap">
			...
			class="gui-editor-img-error gui-flex gui-columns gui-justify-content-center">
				<text class="gui-editor-img-error-text gui-block-text gui-text-center gui-icons">{{$t('gui_editor_64')}}</text>
			</view>
		</view>
		<!-- 引用 -->
		<view v-else-if="item.type == 'quote'">
			...
			:value="item.content" :placeholder="$t('gui_editor_65')" />
		</view>
		<!-- 代码 -->
		<view v-else-if="item.type == 'code'">
			...
			style="height:300rpx;"
			:data-index="index" @input="graceEditorInput" :focus="item.focusin" 
			:value="item.content" :placeholder="$t('gui_editor_66')" />
		</view>
		<!-- 加粗 -->
		...
		<view v-else-if="item.type == 'link'">
			...
			:value="item.content" :placeholder="$t('gui_editor_68')" />
		</view>
		<!-- 分割 -->
		...
		<view class="gui-flex gui-rows gui-justify-content-end gui-editor-item-btns-wrap">
			...
			:data-index="index" @tap="moveup">
				<text class="gui-editor-item-btns-text gui-block-text gui-icons">{{$t('gui_editor_69')}}</text>
			</view>
			<view class="gui-editor-item-btns" hover-class="gui-tap" 
			:data-index="index" @tap="movedown">
				<text class="gui-editor-item-btns-text gui-block-text gui-icons">{{$t('gui_editor_70')}}</text>
			</view>
			<view class="gui-editor-item-btns" @tap="deleteItem" 
			hover-class="gui-tap" :data-index="index">
				<text class="gui-editor-item-btns-text gui-block-text gui-icons">{{$t('gui_editor_71')}}</text>
			</view>
		</view>
	</view>
...
export default{
	...
	methods:{
		...
		deleteItem : function(e){
			var index = e.currentTarget.dataset.index;
			uni.showModal({
				title:this.$t('gui_editor_72'),
				content:this.$t('gui_editor_73'),
				success:(e)=>{
