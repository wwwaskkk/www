...
export default {
	...
	data() {
		return {
			...
			// 播放相关
			player          : null,
			playTxt         : this.$t('gui_im_input_78')
		}
	},
	created : function(){
		// #ifndef H5
		...
		this.recorderManager.onStop((res) => {
			...
			this.recing      = false;
			this.recTxt       =  this.$t('gui_im_input_79')+this.recLength+
			this.$t('gui_im_input_80');
			clearInterval(this.recTimer);
		});
		this.recorderManager.onError(() => {
			uni.showToast({ title: this.$t('gui_im_input_81'), icon: 'none' });
			this.recing = false;
			this.recTxt   = this.$t('gui_im_input_77'),
			clearInterval(this.recTimer);
		});
		// #endif
		// #ifdef MP
	...
	methods:{
		...
		rec : function(){
			if (this.recing){
				...
				this.recing     = true;
				this.recTxt     =  this.$t('gui_im_input_82');
				this.recLength  = 1;
		...
		sendVoiceMsg : function(){
			if (this.tmpVoice == '') {
				uni.showToast({ title: this.$t('gui_im_input_83'), icon: "none" });
				return;
			}
			// 关闭界面
			this.recShow = false;
